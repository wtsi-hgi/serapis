...
import unittest
from serapis.domain.models.data import DNAData
...
class TestDNAData(unittest.TestCase):
    ...
    def test_get_metadata_for_samples_from_seqscape(self):
        ...
        results = DNAData.get_metadata_for_samples_from_seqscape(sample_ids)
        ...
        results = DNAData.get_metadata_for_samples_from_seqscape([])
        self.assertEqual(len(results), 0)

        # Test that if the samples doesn't exist in Seqscape, an empty list is returned:
        results = DNAData.get_metadata_for_samples_from_seqscape([('name', '2294STDY53951111111111')])

        # Test that if the sample is given by a wrong type of id, an exception is thrown:
        self.assertRaises(ValueError, DNAData.get_metadata_for_samples_from_seqscape, [('noid', '123')])

        # Test that if None is received as parameter, an exception is thrown:
        self.assertRaises(ValueError, DNAData.get_metadata_for_samples_from_seqscape, None)
    ...
    def test_get_metadata_for_studies_by_samples_from_seqscape(self):
        ...
        results = DNAData.get_metadata_for_studies_by_samples_from_seqscape(sample_internal_ids)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].internal_id, 1948)

        # Testing that it returns [] if the sample doesn't exist:
        results = DNAData.get_metadata_for_studies_by_samples_from_seqscape([99999999])
        self.assertEqual(len(results), 0)

        # Testing that it returns [] if the argument is []
        results = DNAData.get_metadata_for_studies_by_samples_from_seqscape([])
