...
#        raise exceptions.NotEnoughInformationProvided(msg="There was no information regarding the reference genome provided")
    ...
    files_type = check_all_files_same_type(file_et_index_map)
    if not files_type:
    ...
    submission_id = data_access.SubmissionBuilder.build_and_save(submission_data, user_id)
...
def update_file_from_task(submission_id, file_id, data):
    file_logic = app_logic.FileBusinessLogicBuilder.build_from_file(file_id)
    tasks_dict = file_logic.file_data_access.retrieve_tasks_dict(file_id)
    try: 
    ...
    if task_type in constants.IRODS_TASKS:
        file_logic.file_data_access.update_task_status(file_id, task_id=data['task_id'], task_status=data['status'], errors=errors)
    else:
        if data['status'] == constants.FAILURE_STATUS:
            file_logic.file_data_access.update_task_status(file_id, task_id=data['task_id'], task_status=data['status'], errors=errors)
        else:
            file_logic.file_data_access.update_file_mdata(file_id, data['result'], 
                                                  task_type, 
    ...
    if task_type == constants.UPLOAD_FILE_TASK:
        file_to_update = file_logic.file_data_access.retrieve_submitted_file(file_id)
        serapis2irods.serapis2irods_logic.gather_mdata(file_to_update)
        
    file_to_update = file_logic.file_data_access.retrieve_submitted_file(file_id)
    serapis2irods.serapis2irods_logic.gather_mdata(file_to_update)
    file_logic.status_checker.check_and_update_all_statuses(file_id, file_to_update)



def update_file_from_user(submission_id, file_id, data):
    file_logic = app_logic.FileBusinessLogicBuilder.build_from_file(file_id)
    file_to_update = file_logic.file_data_access.retrieve_submitted_file(file_id)
    has_new_entities = False
    if 'library_list' in data and file_logic.file_data_access.check_if_list_has_new_entities(file_to_update.library_list, data['library_list']) == True: 
        logging.debug("Has new libraries!")
        has_new_entities = True
    elif 'sample_list' in data and file_logic.file_data_access.check_if_list_has_new_entities(file_to_update.sample_list, data['sample_list']) == True:
        logging.debug("Has new samples!")
        has_new_entities = True
    elif 'study_list' in data and file_logic.file_data_access.check_if_list_has_new_entities(file_to_update.study_list, data['study_list']) == True:
        logging.debug("Has new studies!")
        has_new_entities = True
    
    file_logic.file_data_access.update_file_mdata(file_id, data, update_source=constants.EXTERNAL_SOURCE)
    if has_new_entities == True:
        ...
        submitted = file_logic.submit_tasks([constants.UPDATE_MDATA_TASK], file_id, file_to_update)
        if not submitted:
            #return False
...
def update_file_submitted(submission_id, file_id, data):
    ...
    if 'task_id' in data:
        update_file_from_task(submission_id, file_id, data)
    else:
        update_file_from_user(submission_id, file_id, data)
...
def resubmit_jobs_for_file(submission_id, file_id, file_to_resubmit=None):
    ...
    result = file_logic.resubmit_failed_tasks(file_id, file_to_resubmit)
    if result:
...
def delete_submitted_file(submission_id, file_id):
    ...
    subm_file = data_access.FileDataAccess.retrieve_submitted_file(file_id)
    ...
    if file_obj_id in submission.files_list:
        ...
        if len(submission.files_list) == 0:
            ...
            data_access.SubmissionDataAccess.update_submission_file_list(submission_id, submission.files_list)
...
def add_library_to_file_mdata(submission_id, file_id, data):
    ...
    if data_access.FileDataAccess.search_JSONLibrary(data, file_id) != None:
        raise exceptions.NoEntityCreated("Library already exists in the list. For update, please send a PUT request.")
    inserted = data_access.FileDataAccess.insert_library_in_db(data, sender, file_id)
    if inserted == True:
...
#        submitted_file.reload()
        #launch_update_file_task(submitted_file)
#        task_id = task_launcher.TaskLauncher.launch_update_file_task(submitted_file)
...
#        db_model_operations.update_file_from_dict(file_id, upd_dict)
        file_obj = data_access.FileDataAccess.retrieve_submitted_file(file_id)
        file_logic = app_logic.FileBusinessLogicBuilder.build_from_file(file_id, file_obj)
...
def delete_library(submission_id, file_id, library_id):
    ...
    '''
    deleted = data_access.FileDataAccess.delete_library(file_id, library_id)
    if deleted == 1:
        db_model_operations.check_and_update_all_file_statuses(file_id)
    return deleted
...
def add_sample_to_file_mdata(submission_id, file_id, data):
    ...
    if inserted == True:
        ...
        submitted_file.reload()
        #launch_update_file_task(submitted_file)
        task_id = launch_update_file_task(submitted_file)
        upd_dict = {'set__tasks_dict__'+task_id+'__status' : constants.PENDING_ON_WORKER_STATUS,
                    'set__tasks_dict__'+task_id+'__type' : update_file_task.name
                    }
        db_model_operations.update_file_from_dict(file_id, upd_dict)
